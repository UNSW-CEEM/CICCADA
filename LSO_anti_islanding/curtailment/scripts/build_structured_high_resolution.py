...
reads local SAPN training/validation and BOM files with Polars instead of
querying Trino.
SAPN2022 training metrology is expected to have already been converted from the
...
- SAPN2022 training rows are labelled dataset_role="train";
...
# machine-specific paths are loaded from the ignored `local_paths.py` file
# instead of being committed.
DEFAULT_SAPN_ROOT = require_local_path(
    "SAPN_ROOT",
    "root folder containing `Nov2022/`, `All Results/`, and `updated results/`.",
)
DEFAULT_SAPN2022_TRAIN_ROOT = require_local_path(
    "SAPN2022_TRAIN_ROOT",
...
SAPN2022_TRAIN_SITE_METADATA_PATH = (
    DEFAULT_SAPN2022_TRAIN_ROOT / "site_metadata.csv"
)
SAPN2022_TRAIN_CIRCUIT_METADATA_PATH = (
    DEFAULT_SAPN2022_TRAIN_ROOT / "circuit_metadata.csv"
)
SAPN2022_TRAIN_DATA_DIR = (
    DEFAULT_SAPN2022_TRAIN_ROOT / "curtailment training data parquet"
)
...
# not on the raw SAPN training/validation or BOM input schemas.
STRUCTURED_COLUMNS = [
    "site_id",
    "t_stamp",
    "actual_day",
...
def run(
    *,
    ...
    # Training uses SAPN2022 training data. Validation/test uses SAPN2022 data.
    # BOM is read
    ...
    # Build the SAPN validation cohort, then reuse the same site IDs for
    # SAPN2022 training so capacity, BOM grid mapping, and later scoring are
    # consistent.
    site_cohort = None
    if USE_SITE_COHORT:
    ...
    # SAPN2022 training metadata use different source column names, but are
    # standardised to site_id/c_id/polarity before this point.
