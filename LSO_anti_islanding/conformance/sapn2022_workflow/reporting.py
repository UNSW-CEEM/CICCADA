...
import polars as pl
from sapn2022_workflow.config import PRIMARY_PHASE_B_METHOD
...
def build_sapn_site_conformance_summary(results):
    ...
    if phase_b_summary.is_empty() or site_thresholds.is_empty():
        raise ValueError(
            "SAPN Phase B summary and threshold tables must contain the same sites."
        )
    ...
    if (
        ...
        raise ValueError(
            "SAPN Phase B summary and threshold tables have different site IDs."
        )
