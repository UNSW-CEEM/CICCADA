...
class LegacyPVBehaviourMixin:
    def passiveIslanding(
        self, PRated, Reso250ms=None, straddle_tol=1, vThreshold1=265, vThreshold2=275
    ):
        ...
        power_cols = [
            c
            for c in self.circuitData.columns
            if c.startswith("power") and not c.endswith("_next")
        ]
        power_cols_next = [
            c
            for c in self.circuitData.columns
            if c.startswith("power") and c.endswith("_next")
        ]
        ...
        df = df.with_columns(
            ((pl.col("OV1")) & (~pl.col("OV1").shift(1).fill_null(False))).alias(
                "enterOV1"
            )
        )
        df = df.with_columns(
            pl.col("enterOV1").cast(pl.Int8).cum_sum().alias("latchOV1")
        )
        df = df.with_columns(
            pl.when(pl.col("OV1"))
            .then(pl.col("latchOV1"))
            .otherwise(None)
            .alias("eventOV1")
        )
        ...
        df = df.with_columns(
            [
                ...
                .then(
                    (
                        pl.col("local_tstamp")
                        - pl.first("local_tstamp").over("eventOV1")
                    ).dt.total_seconds()
                )
                .otherwise(None)
        ...
        df = df.with_columns(
            [
                (
                    pl.col("OV1")
                    & (pl.col("eventsTimeIncrement") > 0)
                    & (pl.col("eventsTimeIncrement") <= 1.0)
                ).alias("T1"),
                (
                    pl.col("OV1")
                    & (pl.col("eventsTimeIncrement") > 1.0)
                    & (pl.col("eventsTimeIncrement") <= 3.0)
                ).alias("T2"),
            ]
        )
        ...
        df = df.with_columns(
            [
                ...
                (pl.max("T1").over("eventOV1") | pl.max("T2").over("eventOV1")).alias(
                    "has3s"
                ),
        ...
        df = (
            df.with_columns(
                ((pl.col("OV2")) & (~pl.col("OV2").shift(1).fill_null(False))).alias(
                    "enterOV2"
                )
            )
        ...
        df = df.with_columns(
            [
                ...
                .then(
                    (
                        pl.col("local_tstamp")
                        - pl.first("local_tstamp").over("eventOV2")
                    ).dt.total_seconds()
                )
                .otherwise(None)
                .alias("t_OV2")
        ...
        df_strict = df_strict.with_columns(
            [
                (
                    ...
                    & (
                        pl.col("local_tstamp")
                        <= pl.col("T0") + pl.duration(seconds=300)
                    )
                ).alias("in_5min"),
                pl.all_horizontal(
                    [
                        (pl.col(c).fill_null(0) <= self.powerMeasError * PRated)
                        for c in [*power_cols]
                    ]
                ).alias("is_disc"),
            ]
        ...
        df_strict = df_strict.with_columns(
            [
                pl.when(pl.col("is_disc"))
                .then(pl.col("eventsTimeIncrement"))
                .otherwise(None)
                .min()
                .over("eventOV1")
                .alias("first_disc_t"),
        ...
        df_strict = df_strict.with_columns(
            [
                pl.when((pl.col("eventsTimeIncrement") <= 3.0) & (~pl.col("is_disc")))
                .then(pl.col("eventsTimeIncrement"))
                .otherwise(None)
                .max()
                .over("eventOV1")
                .alias("texp_before_3s"),
                pl.when((pl.col("eventsTimeIncrement") >= 3.0) & (pl.col("is_disc")))
                .then(pl.col("eventsTimeIncrement"))
                .otherwise(None)
                .min()
                .over("eventOV1")
                .alias("tzero_after_3s"),
            ]
        ).with_columns(
            [
                (pl.col("tzero_after_3s") - pl.col("texp_before_3s")).alias(
                    "straddle_gap_s"
                ),
            ]
        )
        ...
        df_strict = df_strict.with_columns(
            [
                pl.when(~pl.col("OV1"))
                .then(pl.lit(None))
                .when(pl.col("T1") & pl.col("is_disc"))
                .then(pl.lit("non_compliant"))
                .when(pl.col("T1"))
                .then(pl.lit("compliant"))
                .when(pl.col("T2") & pl.col("is_disc"))
                .then(pl.lit("compliant"))
                .when(pl.col("T2") & ~pl.col("is_disc"))
                .then(pl.lit("indeterminate"))
                .when(
                    pl.col("AFTER3s")
                    & (pl.col("first_disc_t") <= 3.0)
                    & pl.col("is_disc")
                )
                .then(pl.lit("compliant"))
                .when(
                    pl.col("AFTER3s")
                    & ~pl.col("AFTER5min")
                    & pl.col("texp_before_3s").is_not_null()
                    & pl.col("tzero_after_3s").is_not_null()
                    & (pl.col("straddle_gap_s") <= straddle_tol)
                    & pl.col("is_disc")
                )
                .then(pl.lit("compliant"))
                .when(
                ...
                .when(
                    pl.col("AFTER3s")
                    & ~pl.col("AFTER5min")
                    & pl.col("first_disc_t").is_null()
                )
                .then(pl.lit("non_compliant"))
                .when(
                    pl.col("AFTER3s")
                    & ~pl.col("AFTER5min")
                    & pl.col("first_disc_t").is_not_null()
                    & (pl.col("first_disc_t") > 3.0)
                    & (pl.col("eventsTimeIncrement") < pl.col("first_disc_t"))
                )
                .then(pl.lit("non_compliant"))
                .when(
                    pl.col("AFTER3s")
                ...
                .then(pl.lit("non_compliant"))
                .when(pl.col("AFTER5min"))
                .then(
                    pl.when(pl.col("first_disc_t").is_null())
                    .then(pl.lit("non_compliant"))
                    .when(pl.col("eventsTimeIncrement") > pl.col("first_disc_t"))
                    .then(
                        pl.when(pl.col("is_disc"))
                        .then(pl.lit("compliant"))
        ...
        df_lenient = (
                ...
                [
                    ...
                    .otherwise(
                        pl.when(
                            pl.all_horizontal(
                                [
                                    (pl.col(c).fill_null(0) <= P_DISCONNECT)
                                    for c in power_cols
                                ]
                            )
                            | pl.all_horizontal(
                                [
                                    (pl.col(c).fill_null(0) <= P_DISCONNECT)
                                    for c in power_cols_next
                                ]
                            )
                        )
                        .then(pl.lit("compliant"))
                        .otherwise(
                            pl.when(pl.col("L") < pl.col("dt_to_next"))
                            .then(pl.lit("indeterminate"))
                            .otherwise(pl.lit("non_compliant"))
                        )
                    )
            ...
            [
                (
                    pl.all_horizontal(
                        [(pl.col(c).fill_null(0) <= P_DISCONNECT) for c in power_cols]
                    )
                    | pl.all_horizontal(
                        [
                            (pl.col(c).fill_null(0) <= P_DISCONNECT)
        ...
        dfOV2 = dfOV2.with_columns(
            ...
            .otherwise(
                pl.when(
                    pl.all_horizontal(
                        [
                            (pl.col(c).fill_null(0) <= P_DISCONNECT).fill_null(False)
                            for c in power_OV2_cols
                        ]
                    )
                )
                .then(pl.lit("compliant"))
                .otherwise(
                    pl.when(pl.col("L_OV2") < pl.col("dt_to_next_OV2"))
                    .then(pl.lit("indeterminate"))
                    .otherwise(pl.lit("non_compliant"))
                )
            )
        ...
        base = pl.concat(
            [
                df_strict.select(
                    pl.col("OV1ComplianceResult").alias("ComplianceResult")
                ),
                df_lenient.select(
                    pl.col("OV1ComplianceResult").alias("ComplianceResult")
                ),
            ...
            how="vertical",
        ).filter(pl.col("ComplianceResult").is_not_null())
        ...
        complianceSummary = base.select(
            [
                ...
                (non_compliant_expr / denominator_expr * 100).alias(
                    "non_compliant_pct"
                ),
                indeterminate_expr.alias("indeterminate"),
                (indeterminate_expr / denominator_expr * 100).alias(
                    "indeterminate_pct"
                ),
                late_response_expr.alias("late_response"),
                (late_response_expr / denominator_expr * 100).alias(
                    "late_response_pct"
                ),
            ]
        )
    ...
    def sustainedOperation(self, PRated, V_NOM_MAX=258, volThreshParam="mean"):
        ...
        for c in voltage_cols:
            rolled = (
                ...
                .with_columns(
                    pl.col(c)
                    .rolling_mean_by(by="local_tstamp", window_size="10m")
                    .alias(f"vmean_rolling_10m{c.replace(volCol, '', 1)}")
                )
                .select(
                    ["local_tstamp", f"vmean_rolling_10m{c.replace(volCol, '', 1)}"]
                )
            )
        ...
        if volThreshParam == "max":
            timeStepsExceedingThreshold = (
                df.filter(
                    pl.any_horizontal([pl.col(c) > vThreshold for c in vmean_cols])
                )
                .select("local_tstamp")
                .n_unique()
            )
            ...
            df = df.with_columns(
                pl.mean_horizontal([pl.col(c) for c in vmean_cols]).alias(
                    "avg_of_vmean_rolling_10m"
                )
            )
            timeStepsExceedingThreshold = (
        ...
        if timeStepAvailable > 0:
            percentageDataAboveThreshold = (
                timeStepsExceedingThreshold / timeStepAvailable * 100
            )
        ...
        if volThreshParam == "max":
            df = df.with_columns(
                pl.any_horizontal([pl.col(c) > vThreshold for c in vmean_cols]).alias(
                    "over_vmax"
                )
            )
        elif volThreshParam == "mean":
            df = df.with_columns(
                (pl.col("avg_of_vmean_rolling_10m") > vThreshold)
                .fill_null(False)
                .alias("over_vmax")
            )
        ...
        df = df.with_columns(
            [
                ...
                ).alias("enter_over_vmax"),
                (
                    (~pl.col("over_vmax"))
                    & (pl.col("over_vmax").shift(1).fill_null(False))
                ).alias("exit_over_vmax"),
            ]
        )
        df = df.with_columns(
            pl.col("enter_over_vmax").cast(pl.Int8).cum_sum().alias("latch_id")
        )
        df = df.with_columns(
            pl.when(pl.col("over_vmax"))
            .then(pl.col("latch_id"))
            .otherwise(None)
            .alias("active_event_id")
        )
        ...
        df = df.with_columns(
            ...
            .then(
                ...
                .then(
                    pl.when(
                        pl.all_horizontal(
                            [
                                (pl.col(c).fill_null(0) <= P_DISCONNECT)
                                | (pl.col(f"{c}_next").fill_null(0) <= P_DISCONNECT)
                                for c in power_cols
                            ]
                        )
                    )
                ...
                .when(
                    (pl.col("dt_next_s") > self.timeWindowForClassification)
                    & (pl.col("dt_next_s") <= 60 * 5)
                )
                .then(
                    pl.when(
                        pl.all_horizontal(
        ...
        df = df.with_columns(
            pl.when(
                (pl.col("active_event_id").is_not_null())
            ...
            .then(
                ...
                .then(pl.lit("ok_state"))
                .otherwise(pl.lit("state_violation"))
            )
            .otherwise(pl.lit(None))
            .alias("state_check")
        )
        ...
        eventsSummary = (
            ...
            .agg(
                [
                    ...
                    (pl.col("entry_check") == "ok_entry")
                    .sum()
                    .alias("entry_compliant_count"),
                    (pl.col("entry_check") == "non_compliant")
                    .sum()
                    .alias("entry_violation_count"),
                    (pl.col("entry_check") == "late_response")
                    .sum()
                    .alias("late_response_count"),
                    (pl.col("entry_check") == "indeterminate")
                    .sum()
                    .alias("indeterminate_entry_count"),
                    (pl.col("state_check") == "state_violation")
                    .sum()
                    .alias("state_violation_count"),
                    (pl.col("state_check") == "ok_state")
                    .sum()
                    .alias("state_compliant_count"),
                    (
                        pl.col("disc_voltage")
                        .filter(pl.col("disc_voltage").is_not_null())
                        .first()
                        .alias("first_disc_voltage")
                    ),
                ]
            )
        )
        ...
        compliant_expr = (
            pl.col("entry_compliant_count") + pl.col("state_compliant_count")
        ).sum()
        compliance_pct = (
            (compliant_expr) / (n_over_rows - n_events + n_singleton_events) * 100
        )
        non_compliant_expr = (
            pl.col("entry_violation_count") + pl.col("state_violation_count")
        ).sum()
        ...
        v10m_avg_snapshot = (
            pl.mean_horizontal([pl.col(c) for c in vmean_cols])
            if vmean_cols
            else pl.lit(None)
        )
        vinst_max_snapshot = (
            pl.max_horizontal([pl.col(c) for c in voltage_cols])
            if voltage_cols
            else pl.lit(None)
        )
        ...
        disc_trans = (
            ...
            .with_columns(
                pl.col("is_disc").shift(1).over("active_event_id").alias("_prev_disc")
            )
            .with_columns(
                pl.coalesce([pl.col("_prev_disc"), pl.lit(False)]).alias("_prev_disc")
            )
            .filter(
                pl.col("active_event_id").is_not_null()
                & (~pl.col("_prev_disc"))
        ...
        connected_rows = (
            df.filter(~pl.col("is_disc"))
            .select(pl.col("local_tstamp").alias("ts_connected"))
            .sort("ts_connected")
        )
        reconnect_after = (
            over_tbl.filter(
                pl.col("had_disconnect") & ~pl.col("t_last_event").is_null()
            )
            .sort("t_last_event")
            .join_asof(
                connected_rows,
                left_on="t_last_event",
                right_on="ts_connected",
                strategy="forward",
            )
        ...
        _rec_after_v10m = (
            ...
            .join(
                reconnect_after,
                left_on=["active_event_id", "local_tstamp"],
                right_on=["active_event_id", "t_rec"],
                how="inner",
            )
            .group_by("active_event_id")
        ...
        diag_disconnect = _disc_snap.join(
            df.select("site_id", "active_event_id").unique(),
            on="active_event_id",
            how="left",
        ).select(
            "site_id",
        ...
        diag_reconnect = (
            reconnect_after.join(_rec_after_v10m, on="active_event_id", how="left")
...
def check_vw_endpoints(
    ...
    *,
    ...
    df_day = df.filter(
        pl.col("p_frac") >= daylight_frac
    )  # filter out data with greater than 20% gen

    # --- Windows at VW1, filter the lower tolerance band ---
    w1 = df_day.filter(
        (pl.col(voltage_col) >= vw1_low) & (pl.col(voltage_col) <= vw1_high)
    )
    ...
    w2 = w2.with_columns(
        (
            ...
            <= exp_w2
            + p_tol_frac
            * s_rated_apparent  # the measurement tol is meant to be % of apparent power in active
            # .is_between(
    ...
    summary = {
        ...
        "pct_compliant_vw1": (
            100.0 * w1.filter(pl.col("compliant_w1")).height / w1.height
        )
        if w1.height
        else None,  # percentage in the test sample complying
        "pct_compliant_vw2": (
            100.0 * w2.filter(pl.col("compliant_w2")).height / w2.height
        )
        if w2.height
        else None,
    }
...
def inferEvmActivation(
    ...
    if "power" not in df.columns or "local_tstamp" not in df.columns:
        raise ValueError(
            "Data frame must contain the specified PRated and local_tstamp."
        )
    ...
    df = df.sort(local_tstamp).with_columns(
        [
            (
    ...
    flatTopThreshold = curtailmentFrac * float(
        PRated
    )  # this is only relevant for PQ events based on the voltage
    ...
    labeled_df = df.with_columns(
        [
            pl.col("_mins")
            .is_between(start_min, end_min, closed="both")
            .alias("isDaylight")
        ]
    )
    ...
    labeled_df = labeled_df.with_columns(
        pl.col("power")
        .rolling_mean(window_size=smoothWindow, min_periods=1)
        .alias("powerSmoothed")
    )
    ...
    nzFlag = day.select(
        (pl.col("powerSmoothed") <= nearZeroThreshold).alias("nearZeroFlag")
    )

    # build run IDs by counting changes in the boolean flag
    nzChange = nzFlag.select(
        pl.col("nearZeroFlag").cast(pl.Int8).diff().abs().fill_null(0).alias("_change")
    )
    ...
    near_zero_runs = [
        {"start": r[0], "end": r[1], "mins": float(r[2]) if r[2] is not None else 0.0}
        for r in nzRuns.iter_rows()
    ]

    sustainedZeroMinMinutes = 240
    sustained_disconnect = any(
        run["mins"] >= sustainedZeroMinMinutes for run in near_zero_runs
    )
