...
from .helper_funcs import add_event_date, ensure_list
...
def summarize_multi_method_site_outputs(
    ...
    for value_col in pivot_specs:
        ...
        rename_map = {
            col: f"{value_col}__{col}" for col in pivot.columns if col != "site_id"
        }
...
def statsLos(complianceLOSResultAll, sitesWithOverVoltage, uniqueLosSites):
    ...
    # not returning it
    sitesCompliancePerc = (
        complianceLOSResultAll.filter(pl.col("compliance_pct") >= 90).height
        / sitesWithOverVoltage
        * 100
    )  # can also use len(limitOfSustainencesites) as denominator
    sitesNonCompliancePerc = (
        complianceLOSResultAll.filter(pl.col("compliance_pct") < 90).height
        / sitesWithOverVoltage
        * 100
    )  # or 100 - sitesCompliancePerc

    # individual site results
    resultIndividualSites = (
        complianceLOSResultAll.group_by("site_id")
        .agg(
            pl.col("compliant").sum().alias("total_compliant"),
        ...
        .with_columns(
            [
                (pl.col("total_compliant") / pl.col("total") * 100).alias(
                    "pct_compliant"
                ),
                (pl.col("total_non_compliant") / pl.col("total") * 100).alias(
                    "pct_non_compliant"
                ),
            ]
        )
    ...
    compliantSitesIndividualPct = (
        resultIndividualSites.filter(pl.col("pct_compliant") >= 90).height
        / uniqueLosSites
        * 100
    )
    nonCompliantSitesIndividualPct = (
        resultIndividualSites.filter(pl.col("pct_compliant") < 90).height
        / uniqueLosSites
        * 100
    )
    ...
    repeatSites = (
        complianceLOSResultAll.group_by("site_id").len().filter(pl.col("len") > 1)
    )
    repeatSitesIds = repeatSites["site_id"].to_list()  # avoid deprecation
    numRepeatSites = repeatSites.height

    repeat_summary = (
        complianceLOSResultAll.filter(pl.col("site_id").is_in(repeatSitesIds))
        .group_by("site_id")
        .agg(
            [
        ...
        .with_columns(
            [
                ...
                ((pl.col("n_compliant") > 0) & (pl.col("n_non_compliant") > 0)).alias(
                    "mixed"
                ),
            ]
        )
...
def reconnectionStats(reconnectionLOSResultAll):
    ...
    # ------------------------------------------------------------
    behavior_summary = (
        reconnectionLOSResultAll.group_by("behavior_tag")
        .len()
        .with_columns(
    ...
    # ------------------------------------------------------------
    reconn = reconnectionLOSResultAll.filter(
        pl.col("behavior_tag") == "Reconnected After Event"
    ...
    range_stats = reconn.select(
        [
            ...
            ((pl.col("reconnection_time") > 5) & (pl.col("reconnection_time") <= 60))
            .mean()
            .alias("pct_5_60s"),
            # Within 1–5 minutes: 60–300s
            ((pl.col("reconnection_time") > 60) & (pl.col("reconnection_time") <= 300))
            .mean()
            .alias("pct_1_5min"),
            # Within 5–15 minutes: 300–900s
            ((pl.col("reconnection_time") > 300) & (pl.col("reconnection_time") <= 900))
            .mean()
            .alias("pct_5_15min"),
            # Within 15–30 minutes: 900–1800s
            (
                (pl.col("reconnection_time") > 900)
                & (pl.col("reconnection_time") <= 1800)
            )
            .mean()
            .alias("pct_15_30min"),
...
def summarize_never_reconnected_outputs(rr: pl.DataFrame) -> dict[str, pl.DataFrame]:
    ...
    def ensure_col(df: pl.DataFrame, name: str, dtype, default):
        return (
            ...
            else df.with_columns(pl.lit(default).cast(dtype).alias(name))
        )
    ...
    rr = rr.with_columns(
        [
            ...
            .alias("trip_n"),
            pl.when(
                (pl.col("V_trip_obs_list").is_not_null())
                & (pl.col("V_trip_obs_list").list.len() > 0)
            )
    ...
    outcome_by_tag = outcome_counts.with_columns(
    ...
    observability_stats = (
        ...
        .select(
            [
                ...
                (pl.when(pl.col("nightfall_flag")).then(1).otherwise(0))
                .sum()
                .alias("nightfall_count"),
    ...
    screen_stats = (
        ...
        .select(
            [
                ...
                pl.col("reconnect_feasible_screen")
                .cast(pl.Int8)
                .mean()
                .alias("reconnect_feasible_screen_rate"),
            ]
        )
    )
    ...
    trip_min_stats = rr.filter(pl.col("trip_n") > 0).select(
        [
...
def site_day_label_mix(df_events: pl.DataFrame) -> pl.DataFrame:
    ...
    df = df_events.with_columns(pl.col("t_last_event").dt.date().alias("event_date"))

    # Boolean tests for each label
    stayed = pl.col("behavior_tag") == "Stayed Connected During Event"
    during_no_switch = (
        pl.col("behavior_tag") == "Reconnected During Event (no switching)"
    )
    during_switch = (
        pl.col("behavior_tag") == "Reconnected During Event (Intermittent Switching)"
    )
    ...
    mix = (
        df.group_by(["site_id", "event_date"])
        .agg(
            [
        ...
        .with_columns(
            [
                (pl.col("n_stayed_connected") / pl.col("num_events")).alias(
                    "pct_stayed_connected"
                ),
                (pl.col("n_reconnected_during_no_switch") / pl.col("num_events")).alias(
                    "pct_reconnected_during_no_switch"
                ),
                (pl.col("n_reconnected_during_switch") / pl.col("num_events")).alias(
                    "pct_reconnected_during_switch"
                ),
                (pl.col("n_reconnected_after") / pl.col("num_events")).alias(
                    "pct_reconnected_after"
                ),
                (pl.col("n_never_reconnected") / pl.col("num_events")).alias(
                    "pct_never_reconnected"
                ),
            ]
        )
...
def site_day_voltage_stats(
    ...
    disc_first = (
        df.filter(pl.col("v_disc_first").is_not_null())
    ...
    disc_all = (
        df.with_columns(ensure_list("v_disc_during").alias("v_disc_during"))
    ...
    rec_during = (
        df.with_columns(ensure_list("v_rec_during").alias("v_rec_during"))
    ...
    rec_after = (
        df.filter(pl.col("v_rec_after").is_not_null())
        .group_by(["site_id", "event_date"])
        .agg(
            [
    ...
    time_kpi = (
        df.filter(
            pl.col("event_duration_sec").is_not_null()
            & pl.col("disc_time_in_event_sec").is_not_null()
        )
        .group_by(["site_id", "event_date"])
        .agg(
            [
    ...
    out = (
        base.join(disc_first, on=["site_id", "event_date"], how="left")
...
def site_voltage_stats(df_events: pl.DataFrame) -> tuple[pl.DataFrame, pl.DataFrame]:
    ...
    # ---------- Disconnection: first per event ----------
    disc_first = (
        df_events.filter(pl.col("v_disc_first").is_not_null())
        .group_by("site_id")
        .agg(
            [
                pl.col("v_disc_first").min().alias("disc_first_min_v"),
    ...
    # ---------- Disconnection: all transitions (flatten v_disc_during) ----------
    disc_all = (
        df_events.with_columns(ensure_list("v_disc_during").alias("v_disc_during"))
    ...
    # ---------- Reconnection: all during-event transitions (flatten v_rec_during) ----------
    rec_during = (
        df_events.with_columns(ensure_list("v_rec_during").alias("v_rec_during"))
    ...
    # ---------- Reconnection: first after-event per event ----------
    rec_after = (
        df_events.filter(pl.col("v_rec_after").is_not_null())
        .group_by("site_id")
        .agg(
            [
                pl.col("v_rec_after").min().alias("rec_after_min_v"),
    ...
    event_dates = (
        df_events.with_columns(pl.col("t_last_event").dt.date().alias("event_date"))
        .group_by("site_id")
    ...
    out = (
        base.join(disc_first, on="site_id", how="left")
...
def filter_sites_by_time_kpi_leq(
    ...
    if missing:
        raise ValueError(
            f"Missing required columns in site_level_out: {sorted(missing)}"
        )

    return (
        site_level_out.filter(
            pl.col("pct_time_disconnected_during_events") <= pct_threshold
        )
...
def build_mixed_compliance_views(
    ...
    kpi_day = site_day_out.with_columns(
        [
    ...
    per_site_day_lists = (
        kpi_day_valid.group_by("site_id")
        .agg(
            [
                pl.col("event_date")
                .filter(pl.col("LABEL_DAY") == "Compliant")
                .unique()
                .sort()
                .alias("COMPLIANT_DATES"),
                pl.col("event_date")
                .filter(pl.col("LABEL_DAY") == "Non-compliant")
                .unique()
                .sort()
                .alias("NON_COMPLIANT_DATES"),
                pl.col("LABEL_DAY").eq("Compliant").sum().alias("n_days_compliant"),
                pl.col("LABEL_DAY")
                .eq("Non-compliant")
                .sum()
                .alias("n_days_non_compliant"),
            ]
        )
        .with_columns(
            [
                (
                    100.0
                    * pl.col("n_days_compliant")
                    / (pl.col("n_days_compliant") + pl.col("n_days_non_compliant"))
                ).alias("pct_days_compliant"),
            ]
        )
        .with_columns(
    ...
    mixed_sites = per_site_day_lists.filter(
        (pl.col("n_days_compliant") > 0) & (pl.col("n_days_non_compliant") > 0)
    ).select("site_id")

    # ---------- C) Event-level KPI + event_date ----------
    events = (
        reconnection_result.with_columns(
        ...
        .with_columns(
            [
                ...
                .then(
                    100.0
                    * pl.col("disc_time_in_event_sec")
                    / pl.col("event_duration_sec")
                )
                .otherwise(pl.lit(None))
                .alias("event_KPI")
            ]
    ...
    same_day_conflict = (
        events.group_by(["site_id", "event_date"])
        .agg(
            [
        ...
        .with_columns(
            [
                pl.when(
                    pl.col("has_Compliant_Event") & pl.col("has_NonCompliant_Event")
                )
                .then(pl.lit("Yes"))
                .otherwise(pl.lit("No"))
    ...
    site_day_time_pct = (
        ...
        .agg(
            [
                ...
                pl.col("event_duration_sec")
                .filter(pl.col("event_KPI") >= T)
                .sum()
                .alias("sum_compliant_time"),
            ]
        )
        .with_columns(
            [
        ...
        .with_columns(
            [
                (100.0 - pl.col("pct_time_compliant_day")).alias(
                    "pct_time_non_compliant_day"
                )
            ]
        )
        .select(
            [
                "site_id",
                "event_date",
                "pct_time_compliant_day",
                "pct_time_non_compliant_day",
            ]
        )
    )
    # Limit day-level time % to Mixed sites
    site_day_time_pct = site_day_time_pct.join(
        mixed_sites, on="site_id", how="inner"
    ).sort(["site_id", "event_date"])

    # ---------- E) Site-day detail (Mixed sites only; exclude "No KPI") ----------
    mixed_sites_day_detail = (
        kpi_day_valid.join(mixed_sites, on="site_id", how="inner")
    ...
    site_time_pct = (
        events.join(mixed_sites, on="site_id", how="inner")
        .group_by("site_id")
        .agg(
            [
        ...
        .with_columns(
            [
                ...
                .then(
                    100.0
                    * pl.col("sum_compliant_time_site")
                    / pl.col("sum_event_time_site")
                )
                .otherwise(pl.lit(None))
        ...
        .with_columns(
            [
                (100.0 - pl.col("pct_time_compliant_site")).alias(
    ...
    mixed_sites_summary = (
        per_site_day_lists.join(mixed_sites, on="site_id", how="inner")
