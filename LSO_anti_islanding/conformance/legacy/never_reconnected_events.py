    ...
    *,
    ...
    def coverage_seconds(df_window: Optional[pl.DataFrame]) -> float:
        ...
        if dur_all <= 0.0:
            return 0.0
        dur_usable = float(
            df_window.filter(usable_mask).select(pl.col("duration").sum()).item() or 0.0
        )
    ...
    def longest_run_seconds(df_window: Optional[pl.DataFrame], bool_col: str) -> float:
        if df_window is None or df_window.is_empty():
            return 0.0
        runs = df_window.with_columns(
            pl.col(bool_col).cast(pl.Int8).alias("flag_i")
        ).with_columns(
    ...
    def total_seconds_where(df_window: Optional[pl.DataFrame], expr: pl.Expr) -> float:
        if df_window is None or df_window.is_empty():
            return 0.0
        return float(
            df_window.filter(expr).select(pl.col("duration").sum()).item() or 0.0
        )

    def first_reconnect_within(
        df_win: Optional[pl.DataFrame], t_last_event, horizon_h: int
    ):
    ...
    def earliest_qualifying_dwell(
        df_window: Optional[pl.DataFrame], V_rec: float, T_rec_s: int
    ) -> Tuple[Optional[Any], float]:
        if df_window is None or df_window.is_empty():
            return (None, 0.0)
        safe_expr = pl.all_horizontal(
            [pl.col(c).fill_null(pl.lit(float("inf"))) <= V_rec for c in vmean_cols]
        )
        win = df_window.with_columns(safe_expr.alias("safe"))
        if win.filter(pl.col("safe")).is_empty():
            return (None, 0.0)
        runs = win.with_columns(
            pl.col("safe").cast(pl.Int8).alias("safe_i")
        ).with_columns(
    ...
    for row in nr_events.iter_rows(named=True):
        ...
        nightfall_flag = (D0_after_cov < MIN_DAYLIGHT_COVERAGE) and (
            D1_cov < MIN_DAYLIGHT_COVERAGE
        )
        ...
        if nightfall_flag:
            ...
            candidates = []
            t_rec_d1 = (
                first_reconnect_within(df_D1, t_last_event, SEARCH_HORIZON_H)
            ...
            t_rec_d2 = (
                first_reconnect_within(df_D2, t_last_event, SEARCH_HORIZON_H)
            ...
            if candidates:
                ...
                behavior_tag_step2 = (
                    "RECONNECTED_NEXT_DAY" if win_tag == "D1" else "RE reconnected_late"
                )
                ...
                def longest_no_ov(win):
                    return (
                        ...
                        else longest_run_seconds(
                            win.with_columns((~pl.col("over_vmax")).alias("not_ov")),
                            "not_ov",
                        )
                    )
                ...
                reconnect_feasible_screen = bool(
                    no_OV_longest_s >= NO_OV_SCREEN_SECONDS
                )

                if any(
                    w is not None and not w.is_empty()
                    for w in [df_D0_after, df_D1, df_D2]
                ):
                    V_rec_ref = Vmax - 1.0
                    dur_ov_total, dur_stable_ref = 0.0, 0.0
                    for w in [
                        x
                        for x in [df_D0_after, df_D1, df_D2]
                        ...
                        dur_stable_ref += total_seconds_where(
                            ...
                            pl.all_horizontal(
                                [
                                    pl.col(c).fill_null(pl.lit(float("inf")))
                                    <= V_rec_ref
                                    for c in vmean_cols
                                ]
                            ),
                        )
                    if (dur_ov_total > 0.0) and (
                        dur_stable_ref < INTERMITTENT_STABLE_TOTAL_S
                    ):
                ...
                for win_name, df_win in [
                    ("D0_after", df_D0_after),
                    ("D1", df_D1),
                    ("D2", df_D2),
                ]:
                    if df_win is None or df_win.is_empty():
                    ...
                    for DELTA in DELTA_LIST:
                        ...
                        start_ts, dur_s = earliest_qualifying_dwell(
                            df_win, V_rec_cur, T_REC_S
                        )
        ...
        if (behavior_tag_final == "Never Reconnected") and dwell_found:
            ...
            df_ev = df.filter(pl.col("active_event_id") == pl.lit(ev_id)).with_columns(
                [
                    ...
                    .fill_null(False)
                    .cast(pl.Int8)
                    .alias("is_disc_prev_i"),
                    rowwise_vmax_expr().alias("v_obs"),
                ]
            )
            edge_disc = df_ev.filter(
                (pl.col("is_disc_i") == 1) & (pl.col("is_disc_prev_i") == 0)
            )
            edge_rec = df_ev.filter(
                (pl.col("is_disc_i") == 0) & (pl.col("is_disc_prev_i") == 1)
            )

            V_trip_obs_list = (
                edge_disc.select("v_obs")["v_obs"].to_list()
                if not edge_disc.is_empty()
                else []
            )
            V_rec_obs_list = (
                edge_rec.select("v_obs")["v_obs"].to_list()
            ...
            if not V_trip_obs_list:
                ...
                if df_all.is_empty():
                    ...
                    df_forward = df_all.filter(
                        pl.col("local_tstamp") > pl.lit(t_last_event)
                    )

                    # Did ALL phases go <= V_test at any time?
                    safe_expr_test = pl.all_horizontal(
                        [
                            pl.col(c).fill_null(pl.lit(float("inf"))) <= V_test
                            for c in vmean_cols
                        ]
                    )
                    df_forward = df_forward.with_columns(
                        safe_expr_test.alias("safe_below_min_trip")
                    )
                    ...
                    if below_rows.is_empty():
                        ...
                        if not edge_rec.is_empty():
                            has_rec_after = bool(
                                edge_rec.filter(
                                    pl.col("local_tstamp") > pl.lit(t_safe)
                                ).height
                                > 0
                            )
                        behavior_tag_final = (
                            "NORMAL_RECONNECT_BEHAVIOUR"
                            if has_rec_after
                            else "LOCKOUT/PROTECTION_HOLD"
                        )

            # We also keep the lists for audit/metadata
            trip_stats = _stats = _rec_stats = (
                None  # (we won't compute spread unless you want)
            )
        else:
    ...
    updates_df = (
        ...
        if updates
        else pl.DataFrame({"active_event_id": pl.Series([], dtype=pl.Int64)})
    )
    ...
    new_cols = [
        ...
        "D2_coverage",
        "nightfall_flag",
        "reconnect_time_step2",
        "reconnection_time_step2_s",
        "behavior_tag_step2",
        "no_OV_longest_s",
        "reconnect_feasible_screen",
        "context_tag",
        "dwell_found",
        "dwell_first_start",
        "dwell_first_window",
        "dwell_first_hysteresis",
        "V_trip_obs_list",
        "V_rec_obs_list",
        "behavior_tag_final",
    ]
    ...
    reconnection_result = reconnection_result.with_columns(
        pl.col("active_event_id").cast(pl.Int64)
    )
    ...
    rr = rr.with_columns(
        [
            pl.when(
                pl.col("behavior_tag_final").is_not_null()
                & (pl.col("behavior_tag") == "Never Reconnected")
            )
