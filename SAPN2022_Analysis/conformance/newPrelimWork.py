...
sites_orphan_in_metadata = (
    site_details.join(
        circuit_details.select("site_id").unique(),
        on="site_id", how="anti"
    ).select("site_id")
)
...
if numSites == numSitesMeta:
    ...
    sites_with_data = (
        data_df.select("c_id").unique()
            .join(circuit_details.select(["c_id","site_id"]).unique(), on="c_id", how="inner")
    ...
    sites_with_circuits_but_no_data = sites_with_circuits.join(sites_with_data, on="site_id", how="anti")
    # sites_orphan_in_metadata is empty but added for safety
    if sites_with_circuits_but_no_data.height + sites_with_data.height + sites_orphan_in_metadata.height  \
...
pv_sites     = circuit_details.filter(pl.col("con_type") == "pv_site_net").select("site_id").unique()
non_pv_sites = circuit_details.select("site_id").unique().join(pv_sites, on="site_id", how="anti")
...
sites_with_inverter = (
    site_details
    .filter(pl.col("inverter_manufacturer").is_not_null() &
...
if orphan_cids_in_data.height>1:
    print("Num {} of circuits in data but missing in circuit metadata").format(orphan_cids_in_data.height)

# Circuits in metadata with no sites
# other way you could just do is see if they have sites misisng next to them in metadata
circuits_orphan_in_metadata = (
    circuit_details.join(
        site_details.select("site_id").unique(),
        on="site_id", how="anti"
    ).select(["c_id", "site_id"])
)
...
ac_counts = (
    circuit_details
    .filter(pl.col("con_type") == "ac_load_net")
    .group_by("site_id")
...
if pv_counts["pv_count"].sum() != circuit_details.filter(pl.col("con_type")=="pv_site_net")["c_id"].n_unique():
...
circuitsWIth250Reso = circuit_details.filter(pl.col("con_type") == "pv_site_net") \
                        .filter(pl.col("250ms_voltage")==True) \
                        .unique(subset="c_id")

percentageCircuit250Reso = circuitsWIth250Reso.height/ pv_counts["pv_count"].sum() * 100
