...
def process_edp(df, meta_data, offset=9.5 * 60):
    ...
    df5 = (
        df5.groupby(["time", "edp_site_id", "postcode", "Srated"])
        .agg(
            {
                "real_energy": "sum",
                "reactive_energy": "sum",
    ...
    df5["apparent_power"] = df5["apparent_power"] ** 0.5
    df5["P_threshold"] = df5.apply(
        lambda row: get_max_P(row["voltage_avg"], Srated=row["Srated"], v1=253, v2=260),
        axis=1,
    )
    df5["P_noncomp"] = df5["active_power"] - df5["P_threshold"]
    df5["P_noncomp"] = df5["P_noncomp"].clip(lower=0)
    df5["Q_voltvar"] = df5.apply(
        lambda row: get_voltvar_Q(row["voltage_avg"], Srated=row["Srated"]), axis=1
    )
    df5["Q_voltvar_max"] = df5["Q_voltvar"] + df5["Srated"] * 0.05
    df5["Q_voltvar_min"] = df5["Q_voltvar"] - df5["Srated"] * 0.05
    df5["Q_noncomp"] = (
        (df5["reactive_power"] - df5["Q_voltvar"]).abs() - df5["Srated"] * 0.05
    ).abs()
    wrong_meta_data_based_on_maxP = df5.query(f"active_power > Srated")[
        "edp_site_id"
    ].unique()
    df5["wrong_on_maxP"] = False
    df5.loc[
        df5["edp_site_id"].isin(wrong_meta_data_based_on_maxP) == True, "wrong_on_maxP"
    ] = True
