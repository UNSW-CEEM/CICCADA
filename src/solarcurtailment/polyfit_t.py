...
class Polyfit():
    ...
    def filter_power_data_index(self, df):
        ...
        """
        print('Running filter_power_data_index function')
        max_daily_power = df['power'].max()
        print('{0}:{1}'.format('max_daily_power',max_daily_power))
    ...
    def get_datetime_list(self, list_to_convert):
        ...
        """
        # 
        dates = [datetime.strptime(str(d), '%Y-%m-%d %H:%M:%S') for d in list_to_convert]
    ...
    def get_single_date_time(self, d):
        ...
        print("print d")
        print(d)
        return md.date2num(datetime.strptime(str(d), '%Y-%m-%d %H:%M:%S'))
