...
from polyfit_t import Polyfit
from vwatt_curt_t import VWattCurt
...
class VVarCurt():
    ...
    def site_organize(self, c_id_idx, site_details, data, unique_cids):
        print(" VVarCurt() site_organise 3")
    ...
    def check_vvar_curtailment(self, c_id, date, data_site,  ghi, ac_cap, dc_cap, EFF_SYSTEM, is_clear_sky_day):
        print('Running check_vvar_curtailment in vvar_curt_t.py')
       ...
       # dummy = dummy.groupby(level = 0 ).mean()  # average same timestamp values that fall under the same minute category
        ...
        if is_clear_sky_day:
            ...
            print('POLYFIT METHOD TO CALCULATE THE MAX POWER WITHOUT CURTAILMENT, UNAPPLICABLE IN NON CLEAR SKY DAYS')
            circuit_day_data = data_site_complete.reset_index(level=0)
            print(data_site_complete)

            circuit_day_data.rename(columns = {'timestamp':'ts'}, inplace = False)
            circuit_day_data['ts'] = circuit_day_data['timestamp'] 
            print("circuit_day_data columns")
            print(circuit_day_data.columns)
            print("circuit_day_data end")

            #circuit_day_data['ts'] = circuit_day_data['index'].astype(str)
            #circuit_day_data['timestamp'] = circuit_day_data['index'].astype(str)
            df = circuit_day_data
            ...
            print('if it is not clear sky day, use ghi to estimate maximum power without curtailmentz ')
            data_curtailment['max_real_power'] = [min(ghi_t/1000 * dc_cap * EFF_SYSTEM, ac_cap) for ghi_t in ghi_curtailment['Mean global irradiance (over 1 minute) in W/sq m']]
        # =============================================================================================
        ...
        print("Running datasite power_limit_vv computation")
        data_site['power_limit_vv'] = np.sqrt(ac_cap ** 2 - data_site['reactive_power']**2)

        print("data_site['power_limit_vv']")
        print(data_site['power_limit_vv'])
