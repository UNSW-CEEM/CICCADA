...
from bms_sa_review.synthetic_ami_creation.lib import ami_athena as AthenaModule
...
def test_notebook_05_runs_end_to_end_against_synthetic_fixture(monkeypatch, tmp_path):
    ...
    # Derived from TRIAL_MONTHS rather than hardcoded -- this notebook cell's
    # own year is expected to change over time (e.g. widened to include 2024),
    # and this test executes the REAL notebook cells, so it must follow suit
    # rather than hardcoding a year that will drift out of sync.
    trial_year = result["TRIAL_MONTHS"][0]
    ...
    assert inactive_over_history.set_index("circuit_id").loc[231, "months_inactive"] == [
        f"{trial_year}-{m:02d}" for m in range(9, 13)
    ]
    ...
    assert reconstruction_over_history.set_index("site_id").loc[2005, "months_flagged"] == [
        f"{trial_year}-{m:02d}" for m in (11, 12)
    ]
