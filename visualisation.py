...
class my_plot4():
    def __init__(self,t0, t1, df, plt_config, ax_digit, x_label, y_labels, x_format='%a-%m-%d %H:%M', 
        ...
        self.df = self.df.query(f'{self.time_attr} >= \'{self.t0_lc_str}\' and {self.time_attr} < \'{self.t1_lc_str}\'').sort_values(
                by=self.time_attr).reset_index(drop=True)
        ...
        self.plt_loc = np.asarray(np.asarray(list(plt_config.values()))[:, :2], dtype='float32')
        ...
        if self.color_by == 'group':
            ...
            # self.colors = cmap(np.linspace(0, 1, len(self.g_titles)*len(self.plt_attr)))
        if self.plt_t:
            df_tot = self.df.groupby(self.time_attr).agg({self.plt_attr[i]: self.plt_t_func[i] for i in range(len(self.plt_attr))}).reset_index()
            df_tot[self.group_attr] = 'All'
            self.df = pd.concat([self.df, df_tot], axis=0, ignore_index=True).sort_values(by=self.time_attr)
        ...
        self.df_grouped = self.df.groupby(['num_periods', self.group_attr]).agg({i: lambda x:x.to_list() for i in c}).sort_values(by=['num_periods', self.group_attr], ascending=False).reset_index()
    ...
    def get_k_period(self):
        if not (isinstance(self.plt_period, np.timedelta64) or self.plt_period is False):
    ...
    def get_y_max(self, same_scale):
        ...
        if same_scale:
            if self.plt_t:
                ...
                y_max = self.df[self.df[self.group_attr]=='All'][self.plt_attr].max().tolist()
            else:
            ...
            y_max = [ymax_dict[loc]+.1 for loc in y_loc]
    ...
    def get_y_min(self, same_scale):
        ...
        if same_scale:
            if self.plt_t:
                y_min = self.df[self.df[self.group_attr]=='All'][self.plt_attr].min().tolist()
            else:
            ...
            # Step 2: Reassign y_max so each entry reflects the max at its location
            y_min = [y_min_dict[loc]-.1 for loc in y_loc]
        return y_min
    
    def get_yticks(self, same_scale):
        if same_scale:
            yticks = [np.linspace(self.y_min[i], self.y_max[i], self.n_yticks) for i in range(len(self.y_max))]
        else:
            yticks = None
        return yticks
    
    def check_timedelta_E2P(self, MW):
        q_timedelta = self.df.groupby(self.group_attr).agg({self.time_attr: lambda x: x.diff().dropna().unique()}).reset_index(drop=False)
    ...
    def my_fig(self, ncols=1):
        ...
        fig = plt.figure(constrained_layout=True, figsize=[ncols*self.figsize[0], self.k_total*self.figsize[1]], dpi=self.dpi)
    ...
    def color_nights_func(self):
        for i in range(0, self.k_total, np.unique(self.plt_loc[:, 0]).shape[0]):
            t0_dt = self.t0_lc_dt + i/(np.unique(self.plt_loc[:, 0]).shape[0])*self.plt_period
            ...
            while night_t0_num < t1_num:
                ...
                if period_1 > period_0:
                    for ii in range(np.unique(self.plt_loc[:, 0]).shape[0]):
                        self.axs[i+ii].axvspan(period_0, period_1, alpha=0.1, color='black', label='Night Time')
    ...
    def do(self):
        ...
        for k in range(0, self.k_total, num_plots_per):
            ...
            df_part = self.df_grouped[self.df_grouped['num_periods']==num_periods[k_period]]
            time_axis = df_part[self.time_attr]
            time_axis = np.asarray(time_axis)[0]
            xticks = [time_axis[i] for i in np.ceil(np.linspace(0, len(time_axis)-1, self.n_ticks, endpoint=True)).astype(int)]
            xlabels = [time_axis[i].strftime(self.x_format) for i in np.ceil(np.linspace(0, len(time_axis)-1, self.n_ticks, endpoint=True)).astype(int)]
            j_color = 0
            ...
            # for  plt_attr in self.plt_attr:
                ...
                # for g_title in self.g_titles:
                    ...
                    if ax_loc==0:
                        ...
                        if self.step:
                            ax.step(time_axis, y_axis, where='post', color = linecolor, linestyle=linestyle)
                        else:
                            ax.plot(time_axis, y_axis, color = linecolor, linestyle=linestyle)
                        ax.grid(linewidth=self.gridwidth[0])
                        if self.onlyntime:
                            if ax==self.axs[-1]:
                                ax.set_xticklabels(xlabels, rotation=self.rotation, ha='right')
                            else:
                                ax.set_xticklabels([], rotation=self.rotation, ha='right')
                        else:
                            ax.set_xticklabels(xlabels, rotation=self.rotation, ha='right')
                    else:
                        ...
                        if ax == self.axs[0]:
                            if t0_dt.date() != t1_dt.date():
                                if len(str(t0_dt.date())[self.title_i:self.title_j]) > 0:
                                    ax.set_title(f'{self.title} {str(t0_dt.date())[self.title_i:self.title_j]}---{str(t1_dt.date())[self.title_i:self.title_j]}')
                            else:
                                ax.set_title(f'{self.title} {str(t0_dt.date())[self.title_i:self.title_j]}')
                    else:
                        if t0_dt.date() != t1_dt.date():
                            if len(str(t0_dt.date())[self.title_i:self.title_j]) > 0:
                                ax.set_title(f'{self.title} {str(t0_dt.date())[self.title_i:self.title_j]}---{str(t1_dt.date())[self.title_i:self.title_j]}')
                    ...
                    if y_min is not None:
                        ...
                        ax.set_ylim(self.y_min[self.plt_attr.index(plt_attr)], self.y_max[self.plt_attr.index(plt_attr)])
    ...
    def set_legends(self):
        ...
        for k in range(0, self.k_total, np.unique(self.plt_loc[:, 0]).shape[0]):
            for i in range(np.unique(self.plt_loc[:, 0]).shape[0]):
                ax_0_idx = np.logical_and(self.plt_loc[:, 0] == i, self.plt_loc[:, 1] ==0)
                ax_1_idx = np.logical_and(self.plt_loc[:, 0] == i, self.plt_loc[:, 1] ==1)
                ...
                if len(lgnds_0) > 0:
                    self.axs[k+i].legend([str(g_title)+self.legend_join+str(lgnd)   for g_title in self.g_titles for lgnd in lgnds_0 ], loc=next(self.lgnd_loc))
